...
facts, and upcoming/active tasks — with a single LLM call. This is never used in
the live conversation loop, so latency here is fine; it replaces the old per-turn
mood-logging + rollup machinery.
"""
...
_SYSTEM_PROMPT = (
    ...
    "the user opens the app. Use what you know about them and their upcoming tasks "
    "to say something specific and encouraging — acknowledge a notable goal or an "
    "approaching deadline and motivate them ('Your marathon's next week — how's "
    "training going?'). 1-2 sentences, friendly and natural, no markdown, no lists. "
