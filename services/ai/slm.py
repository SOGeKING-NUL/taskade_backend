...
def _build_system_prompt() -> str:
    ...
    return (
        ...
        "SPEAKING RESEARCH RESULTS. After `research` returns, do NOT recite the "
        "findings as a list and do NOT read out every venue, date, or URL — the "
        "full details are shown to the user automatically as a card, so you never "
        "need to enumerate them. Give ONE short, natural spoken sentence: how many "
        "you found and the single most relevant one. For example, if research "
        "turns up several marathons, say something like 'I found three half "
        "marathons in Delhi this August and September — the closest is the "
        "Freedom Half Marathon on August 9th,' NOT a bullet list with every venue "
        "and link. End with a brief natural offer, e.g. 'Want me to set a "
        "reminder for one of them?'\n\n"
